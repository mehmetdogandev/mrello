...
export function CreateBoardDialog({
  ...
  const createBoard = api.board.create.useMutation({
    onSuccess: () => {
      utils.board.getByWorkspace.invalidate({ workspaceId })
      onOpenChange(false)
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Yeni bir pano oluşturarak projelerinizi organize edin.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="name">
              Pano Adı <span className="text-destructive">*</span>
            </Label>
            <Input
              id="name"
              type="text"
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              onChange={(e) =>
                setFormData({ ...formData, isPublic: e.target.value })
              }
              className="flex w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
            >
              <option value="false">Özel (Sadece çalışma alanı üyeleri)</option>
